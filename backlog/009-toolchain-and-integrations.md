...
2. Angular esbuild: investigate a postprocess plugin for the application builder
   (`"plugins"` in `angular.json` executes after esbuild; feasibility spike required —
   may need to hook esbuild's `onLoad` for `.ts` via a custom builder). Document the
   supported matrix explicitly (webpack ✓ / esbuild — status).
3. Vitest: publish a thin `defineForgerVitestPlugin()` (vite `transform` hook calling the
   programmatic transformer with a cached program) — feasibility spike after (2).
